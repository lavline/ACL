...
void insert(Cell* c_list, rule* r)
{
	...
	//int id = (((c_id[0] * LAYER_1 + c_id[1]) * LAYER_2 + c_id[2]) * LAYER_3 + c_id[3]) * LAYER_4 + c_id[4];
	//printf("%d %d\n", p->PRI, id);
...
int match(Cell* c_list, message* m)
{
	...
	int res = 0x7FFFFFFF;
	for (int i = 0; i < 2; i++) {
		int id_1 = c_id[0][i] * LAYER_1; // next layer start id
		for (int j = 0; j < 2; j++) {
			int id_2 = (id_1 + c_id[1][j]) * LAYER_2;
			for (int k = 0; k < 2; k++) {
				int id_3 = (id_2 + c_id[2][k]) * LAYER_3;
				for (int v = 0; v < 2; v++) {
					int id_4 = (id_3 + c_id[3][v]) * LAYER_4;
					for (int w = 0; w < 2; w++) {
						int tar_id = id_4 + c_id[4][w]; // the target cell id
